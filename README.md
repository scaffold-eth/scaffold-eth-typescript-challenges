...
> ❗️ NOTE: functions outlined within the `DEX.sol` are what works with the front end of this branch/repo. Also return variable names may need to be specified exactly as outlined within the `Solutions/DEX.sol` file. If you are confused, see solutions folder in this repo and/or cross reference with front-end code.
...
```bash
git clone https://github.com/scaffold-eth/scaffold-eth-typescript-challenges challenge-4-dex
cd challenge-4-dex
...
> **⭐️ Also note that there is no curve until you uncomment the specific lines of code at the end of `00_deploy_your_contract.ts`**.
...
🚨 Take a second to understand what these functions are doing if you pasted them into your DEX.sol file in packages/hardhat-ts/contracts:
